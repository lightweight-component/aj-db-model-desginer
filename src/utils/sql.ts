...
import { customTypeDefinition } from "./customTypes";
...
export function generateSql(diagram: SchemaDiagram): string {
  const typeStatements: string[] = [
    ...diagram.enums.map((item) =>
      enumStatement(item.name, item.values, diagram.dialect),
    ),
    ...diagram.customTypes.map((item) =>
      customTypeStatement(
        item.name,
        customTypeDefinition(item),
        diagram.dialect,
      ),
    ),
  ];
  const statements: string[] = diagram.tables.map((table: SchemaTable) =>
  ...
  const indexStatements: string[] = diagram.tables.flatMap(
    (table: SchemaTable) =>
      table.indexes.map((index) => {
        const fields: string[] = index.fieldIds.map(
          (fieldId: string) =>
  ...
  return [...typeStatements, ...statements, ...indexStatements]
    .filter((statement: string) => Boolean(statement))
    .join("\n\n");
}
...
function createTableStatement(
  ...
  const primaryKeys: SchemaField[] = table.fields.filter(
    (field: SchemaField) =>
      field.primary && !(dialect === "sqlite" && field.autoIncrement),
  );
  const lines: string[] = table.fields.map((field: SchemaField) =>
    columnDefinition(field, dialect),
  );

  if (primaryKeys.length > 0)
    lines.push(
      `PRIMARY KEY (${primaryKeys.map((field: SchemaField) => quoteIdentifier(dialect, field.name)).join(", ")})`,
    );

  relations
    .filter((relation: SchemaRelation) => relation.sourceTableId === table.id)
    .forEach((relation: SchemaRelation) => {
      const target: SchemaTable | undefined = tables.find(
        (candidate: SchemaTable) => candidate.id === relation.targetTableId,
      );

      if (!target) return;

      const sourceNames: string[] = relation.sourceFieldIds
        .map(
          (fieldId: string) =>
            table.fields.find((field: SchemaField) => field.id === fieldId)
              ?.name,
      ...
      const targetNames: string[] = relation.targetFieldIds
        .map(
          (fieldId: string) =>
      ...
      if (
        sourceNames.length !== relation.sourceFieldIds.length ||
        targetNames.length !== relation.targetFieldIds.length
      )
        return;

      const constraintName: string =
        relation.constraintName.trim() ||
        `fk_${table.name}_${relation.id.slice(0, 8)}`;
      lines.push(
        `CONSTRAINT ${quoteIdentifier(dialect, constraintName)} FOREIGN KEY (${sourceNames.map((name: string) => quoteIdentifier(dialect, name)).join(", ")}) REFERENCES ${quoteIdentifier(dialect, target.name)} (${targetNames.map((name: string) => quoteIdentifier(dialect, name)).join(", ")}) ON DELETE ${relation.onDelete} ON UPDATE ${relation.onUpdate}`,
      );
    });
...
function columnDefinition(
  ...
  if (field.unsigned && (dialect === "mysql" || dialect === "mariadb"))
    parts.push("UNSIGNED");
  ...
  if (field.autoIncrement) {
    ...
    else if (dialect === "sqlite" && field.primary)
...
function quoteIdentifier(dialect: DatabaseDialect, identifier: string): string {
  ...
  if (dialect === "sqlserver") return `[${identifier.split("]").join("]]")}]`;
